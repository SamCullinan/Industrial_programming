...
int Stack_construct(Stack_t * This, int size)
{
	...
	{
		printf("\nТы получил указатель на уже созданный стек!!\n");
...
int  Stack_destruct(Stack_t * This)
{
	...
	if (Stack_OK(This))
	{
		...
		printf("Этот стек удален.Возможно он не существовал!!!");
		ASSERT_OK(This);
		return 1;
	}
}
...
int  Stack_push(Stack_t * This, int value)
{
	...
	if (This->count >= This->max)
	{
		...
		printf("Нельзя добавить элемент!!!\n");
		abort();
	}
...
type Stack_pop(Stack_t * This)
{
	...
	if (!This->count)
	{
		printf("Произошла ошибка при обращении к элементу\n");
		ASSERT_OK(This);
	}
